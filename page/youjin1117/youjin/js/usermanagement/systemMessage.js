...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=UserManage&s=getSysNoticeList&is_open=0&start="+star+"&limit="+evSize;
            break;
        case 1:
            url=relational.url+"?m=UserManage&s=getSysNoticeList&start="+star+"&limit="+evSize;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=UserManage&s=getSysNoticeList&is_open=0&bank="+numeber+"&start="+star+"&limit="+size;
            break;
        case 1:
            url=relational.url+"?m=UserManage&s=getSysNoticeList&bank="+numeber+"&start="+star+"&limit="+size;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=UserManage&s=getSysNoticeInfo&id="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    ...
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=UserManage&s=addSysNotice",
            data:data,
            success: ajaxAddScs,
        }
    )
