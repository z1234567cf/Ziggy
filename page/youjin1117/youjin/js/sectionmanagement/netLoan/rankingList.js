...
function dataClassify(data) {
    ...
    _1=[
        {"cdid":data["cdid"], "zcf_name":"cdid",},
        {"list_date":data["list_date"], "zcf_name":"list_date",},
    ];
    _2=[
        {"ptname":data["ptname"], "zcf_name":"ptname",},
    ];
    _3=[
        {"trade":data["trade"], "zcf_name":"trade",},
    ];
    _4=[
        {"apr":data["apr"], "zcf_name":"apr",},
    ];
    _5=[
        {"borrower_num":data["borrower_num"], "zcf_name":"borrower_num",},
    ];
    _6=[
        {"tender_num":data["tender_num"], "zcf_name":"tender_num",},
    ];
    var _data=[_1,_2,_3,_4,_5,_6];
    return _data;
}
/*
 * ajax类
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Chedai&s=getChedaiList&is_open=0&start="+star+"&limit="+evSize;
            break;
        case 1:
            url=relational.url+"?m=Chedai&s=getChedaiList&is_open=1&start="+star+"&limit="+evSize;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Chedai&s=getChedaiList&is_open=0&name="+numeber+"&start="+star+"&limit="+size;
            break;
        case 1:
            url=relational.url+"?m=Chedai&s=getChedaiList&is_open=1&name="+numeber+"&start="+star+"&limit="+size;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Chedai&s=getChedaiInfo&cdid="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Chedai&s=addChedaiPt",
            data:data,
            success: ajaxAddScs,
        }
    )
    ...
    $.ajax(
        {
            ...
            success: function (msg) {
                ...
                for(i=0;i<length;i++){
                    var li=youJin.makeDom(data[i],ul,"ptid");
                    li.click(function (e) {
                        var e=e||event,
                            _this=this;
                        youJin.liClickHandle(_this,"ptid");
                    })
                }
            },
        }
