...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wj&s=getZqPtList&is_open=0&start="+star+"&limit="+evSize+"&wjid="+relational.spid;
            break;
        case 1:
            url=relational.url+"?m=Wj&s=getZqPtList&is_open=1&start="+star+"&limit="+evSize+"&wjid="+relational.spid;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wj&s=getZqPtList&is_open=0&name="+numeber+"&start="+star+"&limit="+size+"&wjid="+relational.spid;
            break;
        case 1:
            url=relational.url+"?m=Wj&s=getZqPtList&is_open=1&name="+numeber+"&start="+star+"&limit="+size+"&wjid="+relational.spid;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Wj&s=getZqPtInfo&wzid="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    ...
    data["wjid"]=relational.spid;
    // data["pboid"]=relational.pboid;
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Wj&s=addZqPt",
