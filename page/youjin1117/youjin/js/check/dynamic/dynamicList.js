...
function dataClassify(data) {
    ...
    var _data=[_1,_2,_3,_4,_6,_7,_8];
    return _data;
}
/*
 * ajax类
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Manage&s=getNewsList&is_open=0&start="+star+"&limit="+ evSize +
            ...
            break;
        case 1:
            url=relational.url+"?m=Manage&s=getNewsList&is_open=1&start="+star+"&limit="+evSize +
                //"&type="+relational.type+
                //"&is_child="+relational.is_child+
                "&time_start="+relational.time_start+
                "&time_end="+relational.time_end;
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Manage&s=getNewsList&is_open=0&mobile="+numeber+"&start="+star+"&limit="+size+
                //"&type="+relational.type+
                //"&is_child="+relational.is_child+
                "&time_start="+relational.time_start+
                "&time_end="+relational.time_end;
            break;
        case 1:
            url=relational.url+"?m=Manage&s=getNewsList&is_open=1&mobile="+numeber+"&start="+star+"&limit="+size+
                //"&type="+relational.type+
                //"&is_child="+relational.is_child+
                "&time_start="+relational.time_start+
                "&time_end="+relational.time_end;
...
function ajaxAdd(data,id) {
    ...
    if(id=="#zcf_new_pop_debitCard"){
        _url=relational.url+"?m=Manage&s=saveNews";
        data[relational.label]=relational.uid;
    }
    if(id=="#zcf_new_pop_addRelation"){
        _url=relational.url+"?m=Manage&s=delNews";
        data[relational.label]=relational.uid;
    }
...
function ajaxGetValue(id) {
    $.ajax(
        {
            type: "GET",
            url:relational.url+"?m=Manage&s=getNewsInfo&nid="+relational.uid,
            success: function (msg) {
                var _msg=msg;
