...
function dataClassify(data) {
    ...
    _2=[
        {"sum_money":data["sum_money"], "zcf_name":"sum_money",},
    ];
    var _data=[_1,_2];
    return _data;
}
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=AdManage&s=getPostionSumInfo&is_open=0&start="+star+"&limit="+evSize;
            break;
        case 1:
            url=relational.url+"?m=AdManage&s=getPostionSumInfo&is_open=1&start="+star+"&limit="+evSize;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=AdManage&s=getPostionSumInfo&is_open=0&name="+numeber+"&start="+star+"&limit="+size;
            break;
        case 1:
            url=relational.url+"?m=AdManage&s=getPostionSumInfo&is_open=1&name="+numeber+"&start="+star+"&limit="+size;
            break;
    }
    $.ajax({
        type: "GET",
