...
function dataClassify(data) {
    ...
    _3=[
        {"zcf_pid":data["zcf_pid"], "zcf_name":"zcf_pid",},
    ];
    _4=[
        {"desc":data["desc"], "zcf_name":"desc",},
    ];
    _5=[
    ...
    // ];
    var _data=[_1,_4,_3,_2,_5,_6,_7,_8];
    return _data;
}
/*
 * ajax类
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Manage&s=getDianpingList&is_open=0&start="+star+"&limit="+ evSize +
            ...
            break;
        case 1:
            url=relational.url+"?m=Manage&s=getDianpingList&is_open=1&start="+star+"&limit="+evSize +
                "&type="+relational.type+
                "&is_child="+relational.is_child+
                "&time_start="+relational.time_start+
                "&time_end="+relational.time_end;
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Manage&s=getDianpingList&is_open=0&mobile="+numeber+"&start="+star+"&limit="+size+
                "&type="+relational.type+
                "&is_child="+relational.is_child+
                "&time_start="+relational.time_start+
                "&time_end="+relational.time_end;
            break;
        case 1:
            url=relational.url+"?m=Manage&s=getDianpingList&is_open=1&mobile="+numeber+"&start="+star+"&limit="+size+
                "&type="+relational.type+
                "&is_child="+relational.is_child+
                "&time_start="+relational.time_start+
                "&time_end="+relational.time_end;
...
function ajaxGetValue(id) {
    $.ajax(
        {
            type: "GET",
            url:relational.url+"?m=Manage&s=getDianpingInfo&pid="+relational.uid,
            success: function (msg) {
                var _msg=msg;
...
function readyHandle() {
    ...
    $(".setUl li").click(function (e) {
        ...
        if(_this.id=="zcf_modify"){
            ...
            //     par=$("#zcf_container").parent();
            // // par.data("UeditorR",false);
            // par[0].chioce=false;
            // console.log("par",par,"zcf_container",$("#zcf_container"));
