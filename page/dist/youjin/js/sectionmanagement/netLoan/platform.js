...
function originalDataOperate(data) {
    var _data = data;
    _data["zcf_company_name"]=_data["company_name"]+"["+_data["legal_person"]+"]";
    if(_data["apr_min"]&&_data["apr_max"]){
        _data["zcf_rate"]=_data["apr_min"]+"%"+"-"+_data["apr_max"]+"%";
    }else if(!_data["apr_min"]||!_data["apr_max"]){
        _data["zcf_rate"]="";
    }
    return _data;
}
...
function dataClassify(data) {
    ...
    _1=[
        ...
        {"zcf_company_name":data["zcf_company_name"], "zcf_name":"zcf_company_name",},
        {"begin_time":data["begin_time"], "zcf_name":"begin_time",},
    ];
    _2=[
        {"url":data["url"], "zcf_name":"url",},
        {"serve_mobile":data["serve_mobile"], "zcf_name":"serve_mobile",},
        {"public_weixin":data["public_weixin"], "zcf_name":"public_weixin",},
    ];
    // _3=[
    ...
    //     {"xianshi":data["xianshi"], "zcf_name":"xianshi",},
    // ];
    var _data=[_1,_2,_4,_5,_6,_7,_8,_9];
    return _data;
}
/*
 * ajax类
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wd&s=getWdList&is_open=0&start="+star+"&limit="+evSize+
                "&background="+relational.background+
                "&bus_model="+relational.bus_model+
                "&cgid="+relational.cgid;
            break;
        case 1:
            url=relational.url+"?m=Wd&s=getWdList&is_open=1&start="+star+"&limit="+evSize+
                "&background="+relational.background+
                "&bus_model="+relational.bus_model+
                "&cgid="+relational.cgid;
            break;
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wd&s=getWdList&is_open=0&name="+numeber+"&start="+star+"&limit="+size+
                "&background="+relational.background+
                "&bus_model="+relational.bus_model+
                "&cgid="+relational.cgid;
            break;
        case 1:
            url=relational.url+"?m=Wd&s=getWdList&is_open=1&name="+numeber+"&start="+star+"&limit="+size+
                "&background="+relational.background+
                "&bus_model="+relational.bus_model+
                "&cgid="+relational.cgid;
            break;
    ...
    $.ajax(
        {
            ...
            success: function (msg) {
                var bank=$(top.document.body).find("#zcf_new_pop_debitCard [data-base='cgid']"),
                    data=msg.data;
                bank.children().remove();
                data.unshift({
                    cgid:"",
                ...
                domOption(data,bank,"cgid","bank_name");
            },
        }
    )
}
...
function ajaxAdd(data) {
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Wd&s=addWd",
            data:data,
            success: ajaxAddScs,
        }
    )
