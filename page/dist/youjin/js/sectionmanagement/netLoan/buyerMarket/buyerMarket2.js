...
function dataClassify(data) {
    ...
    _1=[
        {"btid":data["btid"], "zcf_name":"btid",},
        {"ptname":data["ptname"], "zcf_name":"ptname",},
    ];
    _2=[
        {"tot":data["tot"], "zcf_name":"tot",},
    ];
    _3=[
        {"last_month_tot":data["last_month_tot"], "zcf_name":"last_month_tot",},
    ];
    // _4=[
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wj&s=getBmTouziList&is_open=0&start="+star+"&limit="+evSize+"&bmid="+relational.spid;
            break;
        case 1:
            url=relational.url+"?m=Wj&s=getBmTouziList&is_open=1&start="+star+"&limit="+evSize+"&bmid="+relational.spid;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wj&s=getBmTouziList&is_open=0&name="+numeber+"&start="+star+"&limit="+size+"&bmid="+relational.spid;
            break;
        case 1:
            url=relational.url+"?m=Wj&s=getBmTouziList&is_open=1&name="+numeber+"&start="+star+"&limit="+size+"&bmid="+relational.spid;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Wj&s=getBmTouziInfo&btid="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    ...
    data["bmid"]=relational.spid;
    // data["pboid"]=relational.pboid;
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Wj&s=addBmTouziList",
            // +"&spid="+relational.spid,
            data:data,
            success: ajaxAddScs,
        }
    )
