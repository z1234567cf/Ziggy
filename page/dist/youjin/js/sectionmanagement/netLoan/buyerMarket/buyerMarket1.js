...
function dataClassify(data) {
    ...
    _4=[
        {"zaitou_count":data["zaitou_count"], "zcf_name":"zaitou_count",},
    ];
    _5=[
        {"focus_scale":data["focus_scale"], "zcf_name":"focus_scale",},
    ];
    _6=[
        {"new_bad":data["new_bad"], "zcf_name":"new_bad",},
    ];
    var _data=[_1,_2,_3];
    return _data;
}
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wj&s=getBmPtList&is_open=0&start="+star+"&limit="+evSize;
            break;
        case 1:
            url=relational.url+"?m=Wj&s=getBmPtList&is_open=1&start="+star+"&limit="+evSize;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wj&s=getBmPtList&is_open=0&name="+numeber+"&start="+star+"&limit="+size;
            break;
        case 1:
            url=relational.url+"?m=Wj&s=getBmPtList&is_open=1&name="+numeber+"&start="+star+"&limit="+size;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Wj&s=getBmPtInfo&bmid="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Wj&s=addBmPt",
            data:data,
            success: ajaxAddScs,
        }
    )
