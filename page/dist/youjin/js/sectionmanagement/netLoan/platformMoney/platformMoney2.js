...
function dataClassify(data) {
    ...
    _1=[
        {"rzid":data["rzid"], "zcf_name":"rzid",},
        {"rongzi_date":data["rongzi_date"], "zcf_name":"rongzi_date",},
    ];
    _2=[
        {"level":data["level"], "zcf_name":"level",},
    ];
    _3=[
        {"money":data["money"], "zcf_name":"money",},
    ];
    _4=[
        {"tender":data["tender"], "zcf_name":"tender",},
    ];
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wd&s=getWdRongzilist&is_open=0&start="+star+"&limit="+evSize+"&ptid="+relational.ptid;
            break;
        case 1:
            url=relational.url+"?m=Wd&s=getWdRongzilist&is_open=1&start="+star+"&limit="+evSize+"&ptid="+relational.ptid;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Wd&s=getWdRongzilist&is_open=0&name="+numeber+"&start="+star+"&limit="+size+"&ptid="+relational.ptid;
            break;
        case 1:
            url=relational.url+"?m=Wd&s=getWdRongzilist&is_open=1&name="+numeber+"&start="+star+"&limit="+size+"&ptid="+relational.ptid;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Wd&s=getWdRongziInfo&rzid="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    ...
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Wd&s=addWdRongzi",
            data:data,
            success: ajaxAddScs,
        }
    )
