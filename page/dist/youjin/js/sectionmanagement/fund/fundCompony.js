...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Jijin&s=getJinjinCompanyList&is_open=0&start="+star+"&limit="+evSize;
            break;
        case 1:
            url=relational.url+"?m=Jijin&s=getJinjinCompanyList&is_open=1&start="+star+"&limit="+evSize;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Jijin&s=getJinjinCompanyList&is_open=0&name="+numeber+"&start="+star+"&limit="+size;
            break;
        case 1:
            url=relational.url+"?m=Jijin&s=getJinjinCompanyList&is_open=1&name="+numeber+"&start="+star+"&limit="+size;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Jijin&s=getJinjinCompanyInfo&jjid="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Jijin&s=addJinjinCompany",
            data:data,
            success: ajaxAddScs,
        }
    )
    ...
    $(".setUl li").click(function (e) {
        ...
        if(_this.id=="uploadLogo"||_this.id=="upload"||_this.id=="uploadBC"){
            ...
            if(_this.id=="uploadLogo"){
                relational.type_id="10";
            }else if(_this.id=="upload"){
                relational.type_id="11";
