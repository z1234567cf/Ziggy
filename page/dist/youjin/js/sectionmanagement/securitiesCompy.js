...
relational.label="stid";
relational.dataTypeID="spid";//删除图像是要取DATA下的什么的类型ID（跟上传图片有关的参数）例如："link_table_id": "wpid",
//数据初步处理过程
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Gupiao&s=getStockCompanyList&is_open=0&start="+star+"&limit="+evSize;
            break;
        case 1:
            url=relational.url+"?m=Gupiao&s=getStockCompanyList&is_open=1&start="+star+"&limit="+evSize;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Gupiao&s=getStockCompanyList&is_open=0&name="+numeber+"&start="+star+"&limit="+size;
            break;
        case 1:
            url=relational.url+"?m=Gupiao&s=getStockCompanyList&is_open=1&name="+numeber+"&start="+star+"&limit="+size;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Gupiao&s=getStockInfo&stid="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Gupiao&s=addStock",
            data:data,
            success: ajaxAddScs,
        }
    )
    ...
    $(".setUl li").click(function (e) {
        ...
        if(_this.id=="uploadLogo"||_this.id=="upload"||_this.id=="uploadBC"){
            ...
            if(_this.id=="uploadLogo"){
                relational.type_id="6";
            }else if(_this.id=="upload"){
                relational.type_id="7";
