...
function dataClassify(data) {
    ...
    _1=[
        {"lid":data["lid"], "zcf_name":"lid",},
        {"name":data["name"], "zcf_name":"name",},
        {"duijie_pt":data["duijie_pt"], "zcf_name":"duijie_pt",},
    ];
    _2=[
        {"company":data["company"], "zcf_name":"company",},
    ];
    var _data=[_1,_2];
    return _data;
}
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Huoqi&s=getLinkList&is_open=0&start="+star+"&limit="+evSize;
            break;
        case 1:
            url=relational.url+"?m=Huoqi&s=getLinkList&is_open=1&start="+star+"&limit="+evSize;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Huoqi&s=getLinkList&is_open=0&name="+numeber+"&start="+star+"&limit="+size;
            break;
        case 1:
            url=relational.url+"?m=Huoqi&s=getLinkList&is_open=1&name="+numeber+"&start="+star+"&limit="+size;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Huoqi&s=getLinkInfo&lid="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Huoqi&s=addLink",
            data:data,
            success: ajaxAddScs,
        }
    )
...
function ajaxplatformSearch(name,DOM) {
    ...
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Huoqi&s=getFxPt&name="+name,
            success: function (msg) {
                ...
                for(i=0;i<length;i++){
                    var li=youJin.makeDom(data[i],ul,"cid","company");
                    li.click(function (e) {
                        var e=e||event,
                            _this=this;
                        youJin.liClickHandle(_this,"cid","company");
                    })
                }
            },
        }
...
function readyHandle() {
    ...
    //搜索下拉提示
    var ptid=$("#zcf_new_pop_debitCard [data-base='company']");
