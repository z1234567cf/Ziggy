...
function dataClassify(data) {
    ...
    _3=[
        {"show_money":data["show_money"], "zcf_name":"show_money",},
    ];
    _4=[
        {"url":data["url"], "zcf_name":"url",},
    ];
    _5=[
        {"show_eaid":data["show_eaid"], "zcf_name":"show_eaid",},
    ];
...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Qujin&s=getQujinList&is_open=0&start="+star+"&limit="+evSize;
            break;
        case 1:
            url=relational.url+"?m=Qujin&s=getQujinList&is_open=1&start="+star+"&limit="+evSize;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=Qujin&s=getQujinList&is_open=0&name="+numeber+"&start="+star+"&limit="+size;
            break;
        case 1:
            url=relational.url+"?m=Qujin&s=getQujinList&is_open=1&name="+numeber+"&start="+star+"&limit="+size;
            break;
    }
    $.ajax({
        type: "GET",
...
function ajxaGet(jikID) {
    $.ajax(
        {
            type: "GET",
            url: relational.url+"?m=Qujin&s=getQujinInfo&eaid="+jikID,
            success: ajxaGetScs,
        }
    )
}
...
function ajaxAdd(data) {
    $.ajax(
        {
            type: "POST",
            url: relational.url+"?m=Qujin&s=addQujin",
            data:data,
            success: ajaxAddScs,
        }
    )
...
function readyHandle() {
    ...
    //搜索下拉提示
    var ptid=$("#zcf_new_pop_debitCard [data-base='name']");
