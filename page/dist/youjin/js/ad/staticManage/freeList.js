...
function ajaxN(star,evSize) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=AdManage&s=getFreeList&is_open=0&start="+star+"&limit="+evSize;
            break;
        case 1:
            url=relational.url+"?m=AdManage&s=getFreeList&is_open=1&start="+star+"&limit="+evSize;
            break;
    }
    $.ajax({
        type: "GET",
...
function successHandle(msg) {
    ...
    for(var i=relational.star;i<relational.evSize*relational.pageIndex;i++){//自动取一段
...
function ajaxSearch(star,size,numeber) {
    ...
    switch (relational.if_open){
        case 0:
            url= relational.url+"?m=AdManage&s=getFreeList&is_open=0&mobile="+numeber+"&start="+star+"&limit="+size;
            break;
        case 1:
            url=relational.url+"?m=AdManage&s=getFreeList&is_open=1&mobile="+numeber+"&start="+star+"&limit="+size;
            break;
    }
    $.ajax({
        type: "GET",
    ...
    $.ajax(
        {
            ...
            success: function (msg) {
                ...
                for(i=0;i<length;i++){
                    var li=youJin.makeDom(data[i],ul,"cid");
                    li.click(function (e) {
                        var e=e||event,
                            _this=this;
                        youJin.liClickHandle(_this,"cid");
                    })
                }
            },
        }
