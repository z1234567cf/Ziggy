(function() {
  ...
  Morris.Line = (function(_super) {
    ...
    Line.prototype.hoverContentForRow = function(index) {
      var content, j, row, y, _i, _len, _ref,_template;
      row = this.data[index];
      ...
      for (j = _i = 0, _len = _ref.length; _i < _len; j = ++_i) {
        y = _ref[j];
        if(typeof this.options.labels == 'string'){  // add by yzhlinux 使用摸版的方式
            if(_template == '') _template = this.options.labels;
            _template = _template.replace('{'+this.options.ykeys[j]+'}',this.yLabelFormat(y));
            _template = _template.replace('{c_'+j+'}',this.colorFor(row, j, 'label'));
            _template = _template.replace('{'+j+'}',this.yLabelFormat(y));
        }else{
            _template += "<div class='morris-hover-point' style='color: " + (this.colorFor(row, j, 'label')) + "'>\n  " + this.options.labels[j] + ":\n  " + (this.yLabelFormat(y)) + "\n</div>";
        }
      }
      content += _template;
